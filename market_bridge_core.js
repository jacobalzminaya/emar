...
// Busca esta función en MarketBridge o añádela antes de 'updateMainSignal'
MarketBridge.updateLeaderUI = function(v, history) {
    ...
    // Obtener la predicción actual y estadísticas del líder V
    const prediction = this.predictions[v] || "WAIT";
    const stats = this.stats[v];
    const accuracy = (stats && stats.total > 0) ? (stats.hits / stats.total * 100) : 0;
    
    // 1. Actualizar Nombre del Patrón
    bestMatchEl.innerText = `PATRÓN GENÉTICO V${v}`;
    bestMatchEl.style.color = "#00d4ff";

    // 2. Actualizar la Señal Grande (BUY/SELL)
    signalValueEl.innerText = prediction;
    if (prediction === "BUY") {
        signalValueEl.style.color = "#00ffaa"; // Verde neón
    } else if (prediction === "SELL") {
        signalValueEl.style.color = "#ff4d82"; // Rojo/Rosa
    } else {
        signalValueEl.style.color = "#888";
    }

    // 3. Actualizar el % de Confianza (Match)
    confidenceEl.innerText = `${accuracy.toFixed(1)}% MATCH`;
    confidenceEl.style.color = accuracy > 75 ? "#00ffaa" : (accuracy > 50 ? "#ffb400" : "#aaa");
};

MarketBridge.projectStreakContinuation = function() {
    if (this.isLocked || this.isPaused || this.isInTrapRecovery) {
    ...
    const dirText = isBullish ? "POSITIVAS" : "NEGATIVAS";
    ...
    if (streak.count >= 1) {
        ...
        if (this.lastLeaderV && this.lastLeaderV <= 9) {
            ...
            if (acc > 0.70) projectionLevel = Math.min(3, projectionLevel + 1);
        }
    ...
    if (projectionLevel >= 1) {
        const text = `PROYECCIÓN: ${sign}${projectionLevel} VELA${projectionLevel > 1 ? 'S' : ''} ${dirText}`;
        const color = isBullish ? "#00ffaa" : "#ff4d82";
        return { text, color };
    }
