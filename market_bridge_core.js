...
toggleReversal() {
    ...
    try {
        ...
        if (status) {
            // Ahora dirá "AUTO-INV" cuando esté en true y "MANUAL" cuando esté en false
            status.innerText = this.isManualReversal ? 'ON (AUTO)' : 'OFF (MANUAL)';
            
            if (btn) {
                // Color púrpura cuando el bot tiene el control de inversión
    ...
    setupInput() {
        // 1. Manejo de clics en el fondo (Mouse)
        document.addEventListener('mousedown', (e) => {
            // Si el clic es en un botón, salimos para que no se duplique la orden
        ...
        // 2. Manejo específico para tus botones BUY / SELL de la interfaz
        // Usamos los IDs que tienes en tu index.html
        try {
        ...
        // Desactivar el menú contextual (clic derecho) para que no moleste
        document.addEventListener('contextmenu', e => e.preventDefault());
    },

 injectManual(type) {
    // 1. DETERMINAR DIRECCIÓN FINAL (Reversión activa por defecto)
    // Si isManualReversal es true (ON), invierte: A -> B / B -> A
    // Si isManualReversal es false (OFF), mantiene: A -> A / B -> B
    let finalType = type; 
    ...
    // Mensaje descriptivo en consola
    console.log(`[DATA-IN] Dirección: ${finalType === 'A' ? 'BUY' : 'SELL'}`);

    // --- A. PROCESAMIENTO DE DATOS (NO SE DETIENE NUNCA) ---

    // 1. Actualizar Secuencia Global (La IA aprende del dato final ejecutado)
    ...
    // 4. Actualizar Interfaz Visual (Velas y Gráficos)
    ...
    // 5. Análisis de IA (Predicciones en segundo plano)
    let predictedMl = ""; 
    try {
    ...
    // --- B. FILTRO DE EJECUCIÓN (SOLO BLOQUEA EL TRADING) ---
    ...
    if (this.isPaused || (this.calculateVolatility() >= 12)) {
        ...
        return; // <--- Se detiene aquí si hay riesgo, no toca el dinero
    }

    // Nueva lógica anti-manipulación: Chequeo de trampa para bloquear ejecución
    ...
    // Chequeo adicional de escaneo ADN (basado en predicciones y stats)
    ...
    if (isTrapDetected || adnTrap) {
        ...
        this.updateTrapUI();
        this.updateStatsUI();
        return; // Bloquea la ejecución financiera
    }

    // --- C. EJECUCIÓN FINANCIERA (SOLO SI NO HAY PAUSA NI TRAMPA) ---

    // Chequeo de recuperación post-trampa
    if (this.isInTrapRecovery) {
        const globalAgreement = this.checkGlobalAgreement(); // Método nuevo para confluencia alta
        if (!globalAgreement) {
            this.addVisibleLog("EN RECUPERACIÓN POST-TRAMPA - ESPERANDO CONFIRMACIÓN", "#ffb400");
            this.runMultiAnalysis();
            this.updateMainSignal();
            return; // Ignora hasta confluencia alta
        } else {
    ...
    this.processWealth(finalType); // Opera con el dato (invertido o no)
    ...
    // Seguridad de volatilidad media
    ...
    // Nuevo método para chequear confluencia histórica alta
    ...
    toggleHedgeFund() {
        ...
        this.updateHedgeFundToggleUI();
        this.updateMainSignal(); // Refrescar señal inmediatamente
    },
        ...
        try {
            ...
            if (this.isHedgeFundEnabled && this.quantHF.volatilityHistory.length >= 10) {
                try {
                    ...
                    console.error("Error en Hedge Fund engine", e);
                    this.addVisibleLog("Fallo en Quant HF → desactivado temporalmente", "#ff5555");
    ...
    exportData() {
        ...
        for (let v = 3; v <= 20; v++) {
            ...
            csv += `V${v},${s.total},${(s.total>0?(s.hits/s.total*100):0).toFixed(2)}%\n`;
        }
...
updateTrapUI() {
    // 1. Obtenemos la predicción de forma segura
    const predictionResult = this.predictNext();
    
    // 2. Nos aseguramos de que sea un String para poder usar .includes()
    // Si predictionResult es un objeto, intentamos sacar su propiedad .dir o lo convertimos a texto
    const currentPred = typeof predictionResult === 'string' 
        ? predictionResult 
        : (predictionResult?.dir || String(predictionResult || '---'));

    // 3. Ahora el .includes() ya no fallará
...
if (typeof updateLiveSequence !== 'function') {
    window.updateLiveSequence = function() {
        // Lógica básica para actualizar secuencia si es necesario
        console.log('Actualizando secuencia en vivo (fallback)');
        // Puedes expandir esto si es necesario
    };
}
